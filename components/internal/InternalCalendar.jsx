...
function CalendarEvent({ task, onSelect }) {
  ...
  return (
    <button
      ...
      title={task.title}
    >
      <span className={`internal-cal-kind-glyph is-${kind}`} aria-hidden="true" />
      <span className="internal-cal-event-title">{task.title}</span>
    </button>
  );
}
...
function HolidayChip({ holiday, locale }) {
  return (
    <span
      className={`internal-cal-holiday is-${holiday.countryCode.toLowerCase()}`}
...
export default function InternalCalendar({
  ...
  function handleDrop(iso, e) {
    e.preventDefault();
    e.stopPropagation();
    const id = e.dataTransfer.getData('application/x-internal-task-id');
  ...
  return (
    <section className={`internal-calendar${compact ? ' is-compact' : ''}`}>
      <header className="internal-cal-header">
        <div className="internal-cal-nav">
          <button type="button" className="internal-cal-nav-btn" onClick={prevMonth} aria-label={t('hub.internal.prevMonth')}>
            ‹
          </button>
          <h2 className="internal-cal-title">{monthLabel}</h2>
          <button type="button" className="internal-cal-nav-btn" onClick={nextMonth} aria-label={t('hub.internal.nextMonth')}>
            ›
          </button>
        </div>
      </header>

      <div className="internal-cal-month" role="grid" aria-label={monthLabel}>
        <div className="internal-cal-dow-row" role="row">
          {dowLabels.map((d, i) => (
            <div key={`dow-${i}`} className="internal-cal-dow" role="columnheader">
              {d}
            </div>
          ))}
        </div>
        ...
        {weekLayouts.map(({ week, segments, laneCount }) => (
          <div
            key={week[0].iso}
            className={['internal-cal-week', laneCount > 0 && 'has-spans'].filter(Boolean).join(' ')}
            style={laneCount > 0 ? { '--cal-week-span-rows': laneCount } : undefined}
            role="row"
          >
            <div className="internal-cal-week-days">
              {week.map(cell => {
                ...
                return (
                  <div
                    ...
                    tabIndex={0}
                  >
                    <span className={`internal-cal-day-num${isToday ? ' is-today' : ''}`}>{dayNum}</span>
                    <div className="internal-cal-events">
                      ...
                      {overflow > 0 && (
                        <button
                          type="button"
                          className="internal-cal-more"
                          onClick={e => {
                            e.stopPropagation();
                            onFocusDay(iso);
                          }}
                        >
                          +{overflow} {t('hub.internal.more')}
                        </button>
                      )}
                    </div>
                  </div>
            ...
            {laneCount > 0 && (
              <div
                className="internal-cal-week-spans"
      ...
      {focusDay && (
        <div className="internal-cal-day-panel">
          <header className="internal-cal-day-panel-head">
            <div>
              <h3>{formatDayLabel(focusDay)}</h3>
              <p>
                {focusHolidays.length > 0 && showHolidays
                  ? `${focusHolidays.length + focusTasks.length === 1 ? t('hub.internal.itemCountOne') : t('hub.internal.itemCount').replace('{count}', String(focusHolidays.length + focusTasks.length))}${focusHolidays.length ? ` · ${focusHolidays.length} ${t('hub.internal.holidayCount')}` : ''}`
                  : focusTasks.length === 1
                    ? t('hub.internal.taskCountOne')
                    : t('hub.internal.taskCount').replace('{count}', String(focusTasks.length))}
              </p>
            </div>
            <button type="button" className="internal-cal-nav-btn" onClick={() => onFocusDay(null)} aria-label={t('hub.internal.close')}>
              ×
            </button>
          </header>
          {focusHolidays.length > 0 && (
            <ul className="internal-cal-holiday-list">
              {focusHolidays.map(h => (
                <li key={h.id} className="internal-cal-holiday-row">
                  <HolidayChip holiday={h} locale={locale} />
                  <span className="internal-cal-holiday-meta">{h.countryCode}</span>
                </li>
              ))}
            </ul>
          )}
                ...
                <li key={task.id}>
                  <button type="button" className="internal-cal-day-row" onClick={() => onTaskClick(task)}>
                    ...
                    {task.kind === 'event' || task.kind === 'milestone' ? (
                      ...
                      <span className={`internal-status-pill is-${task.status}`}>
                        {t(`hub.internal.status${task.status === 'todo' ? 'Todo' : task.status === 'in_progress' ? 'InProgress' : task.status === 'done' ? 'Done' : ''}`) || task.status}
                      </span>
                    )}
                  </button>
                </li>
