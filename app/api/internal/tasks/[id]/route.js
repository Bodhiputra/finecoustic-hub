...
/** @deprecated Use /api/v1/internal/tasks/:id */
export async function GET(request, context) {
  const res = await getInternalTask(request, context);
  return legacyJson(res, data => ({ task: data?.task }));
}

export async function PATCH(request, context) {
  const res = await patchInternalTask(request, context);
  return legacyJson(res, data => ({ task: data?.task }));
}

export async function DELETE(request, context) {
  const res = await deleteInternalTask(request, context);
