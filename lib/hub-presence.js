...
const FILE = join(DATA_DIR, 'hub-presence.json');
...
export async function touchHubPresence(displayName) {
  ...
  if (useDatabase()) {
    await ensureTable();
    await sql()`
      INSERT INTO hub_presence (name_key, display_name, last_seen)
      VALUES (${key}, ${name}, ${now})
...
export async function listHubOnlinePresence() {
