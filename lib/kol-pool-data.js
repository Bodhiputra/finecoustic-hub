...
import { fetchAllKolFromNotion, notionConfigured } from '@/lib/notion-kol';
import {
  ...
  if (useDatabase()) {
    ...
    await sql()`
      INSERT INTO hub_kol_pool (
        ...
        shipping_postal, shipping_country, shipping_phone, shipping_notes,
        ...
        ${rec.shipping_postal}, ${rec.shipping_country}, ${rec.shipping_phone}, ${rec.shipping_notes},
...
async function upsertNotionRecord(rec, persistedById) {
  ...
  await sql()`
    INSERT INTO hub_kol_pool (
      ...
      ${merged.shipping_postal}, ${merged.shipping_country}, ${merged.shipping_phone}, ${merged.shipping_notes},
...
export async function syncKolPoolFromNotion(actorName = '') {
  ...
  try {
    ...
    const meta = {
      ...
      last_error: e.detail || e.message || 'notion_sync_failed',
    };
  ...
  if (useDatabase()) {
    ...
    for (const rec of records) {
      ...
      normalized.push(applyKolCoreOverrides(merged));
    }
    ...
    for (const staleId of staleIds) {
      await sql()`DELETE FROM hub_kol_pool WHERE notion_page_id = ${staleId}`;
    }
