  ...
  for (let i = 0; i < merged.length; i += BULK_UPSERT_CHUNK) {
    ...
    await sql()`
      INSERT INTO hub_kol_pool (
        ...
        shipping_line1, shipping_line2, shipping_name, shipping_city, shipping_state,
        shipping_postal, shipping_country, shipping_country_code, shipping_phone, shipping_email, shipping_tax_id, shipping_notes,
      ...
      SELECT * FROM UNNEST(
        ...
        ${chunk.map(r => r.shipping_name)}::text[],
        ...
        ${chunk.map(r => r.shipping_tax_id)}::text[],
...
async function ensureHubColumns() {
  ...
  await sql()`ALTER TABLE hub_kol_pool ADD COLUMN IF NOT EXISTS shipping_name TEXT NOT NULL DEFAULT ''`;
  ...
  await sql()`ALTER TABLE hub_kol_pool ADD COLUMN IF NOT EXISTS shipping_tax_id TEXT NOT NULL DEFAULT ''`;
  ...
  if (!tablesReadyPromise) {
    tablesReadyPromise = sql()`
      CREATE TABLE IF NOT EXISTS hub_kol_pool (
        ...
        shipping_tax_id TEXT NOT NULL DEFAULT '',
...
function rowToRecord(row) {
  const base = normalizeKolPoolRecord({
    ...
    shipping_tax_id: row.shipping_tax_id,
...
export async function readKolPoolMetaOnly() {
  if (useDatabase()) {
    return readMetaDb();
  }
  return readFileStore().meta || defaultMeta();
}

/** Slim pool rows for outreach cards / tracking (skips heavy JSON overrides). */
export async function listKolPoolOutreachRecords() {
  if (useDatabase()) {
    await ensureTables();
    const rows = await sql()`
      SELECT
        notion_page_id, channel_name, links, main_platform, country, kol_category, tags,
        shipping_line1, shipping_line2, shipping_name, shipping_city, shipping_state,
        shipping_postal, shipping_country, shipping_country_code, shipping_phone,
        shipping_email, shipping_tax_id, shipping_notes, notion_url, synced_at, collaboration_products
      FROM hub_kol_pool
      ORDER BY channel_name ASC
    `;
    return rows.map(rowToRecord);
  }
  return readFileStore().records.map(rowToRecord);
}
  ...
  if (useDatabase()) {
    ...
    await sql()`
      INSERT INTO hub_kol_pool (
        ...
        ${rec.shipping_line1}, ${rec.shipping_line2}, ${rec.shipping_name}, ${rec.shipping_city}, ${rec.shipping_state},
        ${rec.shipping_postal}, ${rec.shipping_country}, ${rec.shipping_country_code}, ${rec.shipping_phone}, ${rec.shipping_email}, ${rec.shipping_tax_id}, ${rec.shipping_notes},
...
export async function createKolPoolRecord(body = {}) {
  ...
  const record = normalizeKolPoolRecord({
    ...
    shipping_tax_id: payload.shipping_tax_id || '',
  ...
  if (useDatabase()) {
    ...
    await sql()`
      UPDATE hub_kol_pool SET
        ...
        shipping_name = ${merged.shipping_name},
        ...
        shipping_tax_id = ${merged.shipping_tax_id},
