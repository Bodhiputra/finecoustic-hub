...
export function getIssueCapabilities(actor, issue) {
  ...
  return {
    ...
    canEditDates: owner,
    canEditDueDate: isAdmin || owner || worker,
...
export function validateIssueUpdate(current, patch, actor) {
  ...
  if (patch.file_urls !== undefined && !jsonEqual(normalizeFileUrls(patch.file_urls), normalizeFileUrls(current.file_urls))) {
    return { ok: false, reason: 'task_locked' };
  }
  ...
  if (patch.assigned_at !== undefined && patch.assigned_at !== current.assigned_at) {
    const workersChanging =
      patch.workers !== undefined && !workersEqual(nextWorkers, currentWorkers);
    const firstJoin =
      workersChanging &&
      !hasWorkers(currentWorkers) &&
      hasWorkers(nextWorkers) &&
      isUserAmongWorkers({ workers: nextWorkers }, actor?.displayName);
    if (!firstJoin) {
      return { ok: false, reason: 'task_locked' };
    }
  }
  ...
  if (patch.due_at !== undefined && patch.due_at !== current.due_at) {
    if (!caps.canEditDueDate) return { ok: false, reason: 'task_locked' };
  }
