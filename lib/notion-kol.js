...
export function getNotionConfig() {
  const apiKey = stripEnvValue(process.env.NOTION_API_KEY);
  const databaseId = stripEnvValue(process.env.NOTION_KOL_DATABASE_ID);
...
/** Turn Notion API failures into short, user-facing sync error codes/messages. */
export function formatNotionSyncError(error) {
  ...
  if (status === 401 || status === 403 || parsed?.status === 401 || parsed?.status === 403) {
    return 'notion_auth_failed';
  }
  if (status === 404 || parsed?.status === 404 || parsed?.code === 'object_not_found') {
    return 'notion_database_not_found';
  }
  if (error?.message === 'notion_not_configured') return 'notion_not_configured';
  return String(error?.message || parsed?.message || 'notion_sync_failed');
}
