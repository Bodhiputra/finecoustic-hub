...
/** Assignee, creator, and individual meeting attendees — deduped, non-empty. */
export { meetingNotificationRecipients } from '@/lib/meeting-notification-recipients';

/** @deprecated alias */
export function scheduleAlertRecipients(task) {
  return meetingNotificationRecipients(task);
}
  ...
  for (const task of tasks) {
    ...
    if (task.kind === 'meeting') {
      ...
      for (const hours of MEETING_ALERT_HOURS) {
        ...
        for (const name of meetingNotificationRecipients(task)) {
