...
export function getManualQty(ops, sku, warehouse) {
  const manual = ops.inventory_manual?.find(i => i.sku === sku && i.warehouse === warehouse)?.qty;
  if (manual != null) return manual;

  if (warehouse === 'dongguan') {
    const disc = ops.inventory_discrepancy?.find(d => d.sku === sku);
    if (disc?.available_remaining != null) return disc.available_remaining;
  }

  if (warehouse === 'shopify') {
    const movements = (ops.movements || [])
      .filter(m => m.sku === sku && m.warehouse === 'shopify')
      .sort((a, b) => String(b.date || '').localeCompare(String(a.date || '')));
    if (movements[0]?.qty != null) return movements[0].qty;
  }

  return null;
}

/** Axia "available after orders" — source quantity for Shopify push. */
export function axiaAvailableQty(ops, sku) {
  return getManualQty(ops, sku, 'dongguan');
}
...
function waitingCustomerNames(ops, sku) {
  const names = ops.b2b_allocations
    .filter(a => a.sku === sku && allocationStillReserved(ops, a.partner_code))
    .map(a => partnerByCode(ops, a.partner_code)?.name || a.partner_code);
...
export function calcStockReconciliation(ops) {
  ...
  const items = ACTIVE_SKUS.map(sku => {
    const m = metrics[sku];
    const available = m.axiaAfterOrders ?? 0;
    // Shipped and waiting orders both reserve warehouse stock (per Axia)
    const customerCommitted = m.b2bShipped + m.b2bReserved;
    const inWarehouseAccounted =
      m.b2bShipped + m.b2bReserved + m.internal + available;
    const inWarehouseGap = m.warehouseQty - inWarehouseAccounted;
    const calcAvailable = m.warehouseQty - m.b2bAll - m.internal;
    const axiaDelta = m.axiaAfterOrders != null ? m.axiaAfterOrders - calcAvailable : null;
    const waitingCustomers = waitingCustomerNames(ops, sku);

    return {
      sku,
      ...m,
      customerCommitted,
      inWarehouseAccounted,
      inWarehouseGap,
      calcAvailable,
      axiaDelta,
      waitingCustomers,
      overCommitted: inWarehouseGap < 0,
      hasGap:
        inWarehouseGap !== 0 ||
  ...
  return {
    ...
    totalOverCommitted: sum(items, i => Math.max(0, -i.inWarehouseGap)),
  };
}
