...
async function ensureTable() {
  ...
  if (!tableReadyPromise) {
    ...
    `
      ...
      `)
      .then(() => sql()`
        DELETE FROM preorder_survey_responses a
        USING preorder_survey_responses b
        WHERE a.id < b.id
      ...
      `)
      .catch(err => {
        // Unique index may fail on legacy duplicates; app-level dedup still applies.
        if (err?.code !== '23505' && !String(err?.message || '').includes('unique index')) {
          throw err;
        }
        console.warn('[preorder-survey] unique index skipped:', err.message);
      })
      .then(() => {
        tableReady = true;
      })
