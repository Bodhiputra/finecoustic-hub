...
function assigneeName(task) {
  ...
  return assignees[0] || String(task?.assignee || '').trim();
}

export async function notifyTaskAssigneesChanged(task, actor, { prevAssignees = [] } = {}) {
  const prev = getTaskAssignees({ assignees: prevAssignees });
  const next = getTaskAssignees(task);
  if (taskAssigneesEqual(prev, next)) return;

  const prevKeys = new Set(prev.map(name => personKey(name)));
  const actorKey = personKey(actor?.displayName);
  const added = next.filter(name => {
    const key = personKey(name);
    return key && !prevKeys.has(key) && key !== actorKey;
  });
  ...
  await Promise.all(
    added.map(name =>
      createHubNotification({
        ...
        dedupeKey: `assigned:${task.id}:${personKey(name)}:${Date.now()}`,
      })
    )
  );
}
...
export async function notifyTaskAssigned(task, actor, { prevAssignee = '' } = {}) {
  return notifyTaskAssigneesChanged(task, actor, {
    prevAssignees: prevAssignee ? [prevAssignee] : [],
  });
}
...
export async function notifyTaskDiscussionComment(task, comment, actor, { skipNames = [] } = {}) {
  ...
  const recipients = [];
  for (const name of [assignerName(task), ...getTaskAssignees(task)]) {
...
export async function runTaskUpdateSideEffects({ existing, next, actor, patch = {} }) {
  ...
  if (patch.assignee !== undefined || patch.assignees !== undefined) {
    jobs.push(notifyTaskAssigneesChanged(next, actor, {
      prevAssignees: getTaskAssignees(existing),
    }));
  }
