...
import { normalizeBoard, normalizeCampaign, normalizeFlowData, normalizeStatusColumns } from '@/lib/internal-campaigns';
import { ALL_DEPARTMENTS_ID, normalizeDepartmentId } from '@/lib/internal';
...
async function readAllCampaignsRaw() {
  if (useDatabase()) {
    ...
    const rows = await sql()`
      SELECT id, department, name, description, flow_enabled, flow_data, sort_order, created_by, created_at, updated_at
  ...
  if (useDatabase()) {
    ...
    await sql()`
      INSERT INTO internal_campaigns (
        id, department, name, description, flow_enabled, flow_data, sort_order, created_by, created_at, updated_at
      ) VALUES (
        ${normalized.id}, ${normalized.department}, ${normalized.name}, ${normalized.description},
        ${normalized.flow_enabled}, ${JSON.stringify(normalized.flow_data)}::jsonb, ${normalized.sort_order}, ${normalized.created_by},
...
export async function createBoard(input, actor) {
  ...
  let campaign = null;
  if (campaignId) {
    campaign = (await readAllCampaignsRaw()).find(c => c.id === campaignId);
    if (!campaign) {
...
export async function updateBoard(id, patch) {
  ...
  if (patch.status_columns !== undefined) {
    const cols = normalizeStatusColumns(patch.status_columns);
    if (!cols.length) {
  ...
  const now = new Date().toISOString();
  return writeBoard({ ...next, updated_at: now });
}
