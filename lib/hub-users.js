...
import {
  ...
  HUB_ASSIGNABLE_DEPARTMENT_IDS,
  normalizeDepartmentAccess,
} from '@/lib/hub-departments';
import { bustHubTeamMembersCache } from '@/lib/server-ttl-cache';
import { normalizeShowScheduleDashboard } from '@/lib/hub-user-preferences';
import { hashPassword, validatePasswordStrength, verifyPassword } from '@/lib/hub-password';
...
const LEGACY_MEMBER_ROLE = 'member';
const DEFAULT_ROLE = 'associate';

let tableReady = false;
let tableReadyPromise = null;
let roleMigrationDone = false;
...
function listFileHubUsersSorted() {
...
async function ensureTable() {
  ...
  if (!tableReadyPromise) {
    ...
    `;
      ...
      await sql()`ALTER TABLE hub_users ADD COLUMN IF NOT EXISTS role TEXT NOT NULL DEFAULT 'associate'`;
      await sql()`UPDATE hub_users SET role = 'associate' WHERE role = 'member'`;
      await sql()`ALTER TABLE hub_users ALTER COLUMN role SET DEFAULT 'associate'`;
      await sql()`ALTER TABLE hub_users ADD COLUMN IF NOT EXISTS session_gen TEXT NOT NULL DEFAULT ''`;
      await sql()`ALTER TABLE hub_users ADD COLUMN IF NOT EXISTS department_access JSONB NOT NULL DEFAULT '{}'::jsonb`;
      await sql()`ALTER TABLE hub_users ADD COLUMN IF NOT EXISTS show_schedule_dashboard BOOLEAN NOT NULL DEFAULT TRUE`;
      tableReady = true;
    }).catch(err => {
...
function normalizeHubRole(role) {
  const r = String(role || DEFAULT_ROLE).trim().toLowerCase();
  if (r === LEGACY_MEMBER_ROLE) return DEFAULT_ROLE;
  return ROLES.includes(r) ? r : DEFAULT_ROLE;
}

async function migrateLegacyMemberRoles() {
  if (roleMigrationDone) return;
  if (useDatabase()) {
    try {
      await ensureTable();
      roleMigrationDone = true;
    } catch (err) {
      console.error('[hub-users] migrateLegacyMemberRoles', err);
    }
    return;
  }
  ...
  for (const user of users) {
    if (user.role === LEGACY_MEMBER_ROLE) {
      user.role = DEFAULT_ROLE;
      changed = true;
    }
  }
  if (changed) writeFileUsers(users);
  roleMigrationDone = true;
}

function normalizeUser(row) {
  if (!row) return null;
  const role = normalizeHubRole(row.role);
  return {
    id: row.id,
    ...
    department_access: normalizeDepartmentAccess(row.department_access, role),
    show_schedule_dashboard: normalizeShowScheduleDashboard(row.show_schedule_dashboard),
    ...
    try {
      ...
      const rows = await withNeonRetry(() => sql()`
        SELECT id, name_key, display_name, password_hash, role, blocked,
               must_change_password, session_gen, sync_token, department_access,
               show_schedule_dashboard, created_at, updated_at
        FROM hub_users WHERE name_key = ${key} LIMIT 1
      `);
      if (!rows.length) return null;
      return { ...normalizeUser(rows[0]), password_hash: rows[0].password_hash };
    } catch (err) {
      console.error('[findHubUserByName] database unavailable, using file fallback', err);
    }
  }
...
export async function findHubUserById(id) {
  ...
  if (useDatabase()) {
    try {
      ...
      console.error('[findHubUserById] database unavailable, using file fallback', err);
    }
  }
...
export async function listHubUsers() {
  await migrateLegacyMemberRoles();
  if (useDatabase()) {
    try {
      await ensureTable();
      const rows = await withNeonRetry(() => sql()`
        SELECT id, display_name, role, blocked, must_change_password, department_access,
               show_schedule_dashboard, created_at, updated_at
        FROM hub_users ORDER BY created_at DESC
      `);
      return rows.map(row => normalizeUser(row));
    } catch (err) {
      console.error('[listHubUsers] database unavailable, using file fallback', err);
    }
  }

  return listFileHubUsersSorted();
}

/** Display names for assignee picker — active (non-blocked) hub accounts only. */
export async function listActiveTeamMemberNames() {
  const users = await listHubUsers();
  return users
    .filter(u => u && !u.blocked)
    .map(u => String(u.display_name || '').trim())
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
}

/** Assignee pickers scoped to one hub department (e.g. KOL outreach). */
export async function listActiveTeamMemberNamesForDepartment(departmentId) {
  const users = await listHubUsers();
  return users
    .filter(u => {
      ...
      const actorLike = {
        ...
        departmentAccess: u.department_access,
        role: u.role,
      };
      return canAccessDepartment(actorLike, departmentId);
    })
    .map(u => String(u.display_name || '').trim())
    .filter(Boolean)
    .sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
...
function normalizeRole(role) {
  return normalizeHubRole(role);
}

/** Admin-provisioned hub account — individual password, no public signup. */
export async function createHubUserByAdmin(displayName, password, { role = DEFAULT_ROLE, department_access = null } = {}) {
  ...
  const user = {
    ...
    department_access: normalizeDepartmentAccess(department_access, normalizeRole(role)),
  ...
  if (useDatabase()) {
    ...
    await sql()`
      INSERT INTO hub_users (
        ...
        ${JSON.stringify(user.department_access)}::jsonb,
        TRUE, ${user.created_at}, ${user.updated_at}
      )
    `;
  } else {
    const users = readFileUsers();
...
/** Remove every provisioned hub user (master login is separate). */
export async function deleteAllHubUsers() {
  if (useDatabase()) {
    await ensureTable();
    const rows = await sql()`DELETE FROM hub_users RETURNING id`;
    for (const row of rows) clearHubSessionGenCache(row.id);
    return rows.length;
  }

  const count = readFileUsers().length;
  writeFileUsers([]);
  return count;
}
...
export async function updateHubUserDepartmentAccess(id, departmentAccess) {
  const user = await findHubUserById(id);
  if (!user) return null;
  const next = normalizeDepartmentAccess(departmentAccess, user.role);
  if (!HUB_ASSIGNABLE_DEPARTMENT_IDS.some(deptId => next[deptId])) {
    return { error: 'department_required' };
  }
  ...
  if (useDatabase()) {
    try {
      ...
      const rows = await withNeonRetry(() => sql()`
        UPDATE hub_users
        SET department_access = ${JSON.stringify(next)}::jsonb, updated_at = NOW()
        WHERE id = ${id}
        RETURNING id, display_name, role, blocked, must_change_password, department_access,
                  show_schedule_dashboard, created_at, updated_at
      `);
      bustHubTeamMembersCache();
      return rows.length ? normalizeUser(rows[0]) : null;
    } catch (err) {
      console.error('[updateHubUserDepartmentAccess] database error', err);
  ...
  const idx = users.findIndex(u => u.id === id);
  if (idx === -1) return null;
  users[idx].department_access = next;
  users[idx].updated_at = new Date().toISOString();
  writeFileUsers(users);
  bustHubTeamMembersCache();
  return normalizeUser(users[idx]);
...
export async function updateHubUserRole(id, role) {
  const user = await findHubUserById(id);
  if (!user) return null;
  const next = normalizeHubRole(role);
  if (!ROLES.includes(next)) return { error: 'invalid_role' };
  ...
  if (useDatabase()) {
    try {
      ...
      const rows = await withNeonRetry(() => sql()`
        UPDATE hub_users
        SET role = ${next}, updated_at = NOW()
        WHERE id = ${id}
        RETURNING id, display_name, role, blocked, must_change_password, department_access,
                  show_schedule_dashboard, created_at, updated_at
      `);
      if (!rows.length) return null;
      await rotateHubUserSession(id);
      bustHubTeamMembersCache();
      return normalizeUser(rows[0]);
    } catch (err) {
      console.error('[updateHubUserRole] database error', err);
      return { error: 'save_failed' };
    }
  }
...
export async function updateHubUserShowScheduleDashboard(id, show) {
  const user = await findHubUserById(id);
  if (!user) return null;
  const next = normalizeShowScheduleDashboard(show);
  ...
  if (useDatabase()) {
    try {
      ...
      const rows = await withNeonRetry(() => sql()`
        UPDATE hub_users
        SET show_schedule_dashboard = ${next}, updated_at = NOW()
        WHERE id = ${id}
        RETURNING id, display_name, role, blocked, must_change_password, department_access,
                  show_schedule_dashboard, created_at, updated_at
      `);
      ...
      console.error('[updateHubUserShowScheduleDashboard] database error', err);
      return { error: 'save_failed' };
    }
  }
  ...
  const idx = users.findIndex(u => u.id === id);
  if (idx === -1) return null;
  users[idx].show_schedule_dashboard = next;
  users[idx].updated_at = new Date().toISOString();
  writeFileUsers(users);
  return normalizeUser(users[idx]);
}
