import { cache } from 'react';
import { cookies } from 'next/headers';
import { HUB_COOKIE, isAdminSession } from '@/lib/auth';
import { verifyHubTokenForGate, verifyToken, SESSION_REALMS } from '@/lib/session-token';
import { findHubUserById } from '@/lib/hub-users';
import { isManagerRole } from '@/lib/hub-users';
import { fullDepartmentAccess, normalizeDepartmentAccess } from '@/lib/hub-departments';
import { normalizeShowScheduleDashboard } from '@/lib/hub-user-preferences';

export async function readHubSessionFromCookies(cookieStore) {
  const token = cookieStore.get(HUB_COOKIE)?.value;
  return verifyHubTokenForGate(token);
}

/** Server-trusted hub actor — never trust client-sent names for permissions. */
export const resolveHubActor = cache(async function resolveHubActor() {
  const cookieStore = await cookies();
  const isAdmin = await isAdminSession();
  const token = cookieStore.get(HUB_COOKIE)?.value;
  const payload = token ? await verifyHubTokenForGate(token) : null;

  if (!payload?.u && !isAdmin) {
    const light = token ? await verifyToken(token, SESSION_REALMS.HUB) : null;
    if (light?.u) {
      const user = await findHubUserById(light.u);
      if (!user) {
        return { ok: false, reason: 'account_deleted', displayName: '', isAdmin: false };
      }
      if (user.blocked) {
        return { ok: false, reason: 'blocked', displayName: '', isAdmin: false };
      }
      return { ok: false, reason: 'session_revoked', displayName: '', isAdmin: false };
    }
    return { ok: false, reason: 'unauthorized', displayName: '', isAdmin: false };
  }

  if (payload?.u) {
    const user = await findHubUserById(payload.u);
    if (!user) {
      return { ok: false, reason: 'account_deleted', displayName: '', isAdmin: false };
    }
    if (user.blocked) {
      return { ok: false, reason: 'blocked', displayName: '', isAdmin: false };
    }
    return {
      ok: true,
      userId: user.id,
      displayName: user.display_name,
      role: user.role,
      isManager: isAdmin || isManagerRole(user.role),
      isAdmin,
      mustChangePassword: Boolean(user.must_change_password || payload.mcp),
      departmentAccess: normalizeDepartmentAccess(user.department_access, user.role),
      showScheduleDashboard: normalizeShowScheduleDashboard(user.show_schedule_dashboard),
    };
  }

  if (isAdmin) {
    return {
      ok: true,
      userId: '',
      displayName: payload?.d || '',
      role: 'manager',
      isManager: true,
      isAdmin: true,
      mustChangePassword: false,
      departmentAccess: fullDepartmentAccess(),
      showScheduleDashboard: true,
    };
  }

  return { ok: false, reason: 'unauthorized', displayName: '', isAdmin: false };
});

export async function requireHubActor() {
  const actor = await resolveHubActor();
  if (!actor.ok) {
    const err = new Error(actor.reason || 'unauthorized');
    err.status = 401;
    throw err;
  }
  return actor;
}
