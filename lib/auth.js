...
export function isHubAuthEnabled() {
  return Boolean(getHubPassword() || getMasterPassword() || process.env.HUB_PER_USER_AUTH !== '0');
}
...
export async function setHubSessionCookie(
  res,
  { displayName = '', userId = '', sessionGen = '', mustChangePassword = false } = {}
) {
  const extra = {};
  ...
  if (mustChangePassword) extra.mustChangePassword = true;
  res.cookies.set(
    HUB_COOKIE,
    await signToken(SESSION_REALMS.HUB, undefined, extra),
    authCookieOptions(HUB_COOKIE, '/')
  );
}
