...
/** Remove reminder notifications that no longer apply (e.g. KOL moved past arrived). */
export async function clearHubNotificationsForEntity(entityId, entityType = 'task', types = []) {
  const id = String(entityId || '').trim();
  const typeList = [...new Set((types || []).map(t => String(t || '').trim()).filter(Boolean))];
  if (!id || !typeList.length) return 0;

  if (useDatabase()) {
    await ensureTable();
    const rows = await sql()`
      DELETE FROM hub_notifications
      WHERE entity_id = ${id}
        AND entity_type = ${entityType}
        AND type = ANY(${typeList})
      RETURNING id
    `;
    return rows.length;
  }
  ...
  const kept = items.filter(
    n => !(n.entity_id === id && n.entity_type === entityType && typeSet.has(n.type))
  );
  const removed = items.length - kept.length;
  if (removed) writeFileStore(kept);
  return removed;
}

/** Mark pending action notifications resolved (e.g. review request after approve/send back). */
export async function resolveHubActionNotifications(entityId, entityType = 'task', types = ['review_request']) {
  const id = String(entityId || '').trim();
  ...
  if (useDatabase()) {
    await ensureTable();
    const rows = await sql()`
      SELECT id, payload FROM hub_notifications
      WHERE entity_id = ${id}
        AND entity_type = ${entityType}
        AND type = ANY(${types})
        AND read_at IS NULL
    ...
    for (const row of rows) {
      ...
      await sql()`
        UPDATE hub_notifications
        SET read_at = ${now}, payload = ${JSON.stringify({ ...payload, resolved: true, resolved_at: now, requires_action: false })}
        WHERE id = ${row.id}
      `;
      count += 1;
    }
  ...
  let count = 0;
  for (const n of items) {
    if (n.entity_id !== id || n.entity_type !== entityType || !typeSet.has(n.type) || n.read_at) continue;
