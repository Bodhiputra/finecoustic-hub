 ...
 * - Sink / milestone (incoming only): upstream sources follow (tasks below the milestone).
 * - Source root (outgoing only): downstream targets follow (chain above a bottom task).
 */
export function getFlowDragFollowers(nodeId, edges = []) {
  ...
  if (inCount > 0 && outCount === 0) {
    return collectReachable(nodeId, incoming);
  }
  if (inCount === 0 && outCount > 0) {
    return collectReachable(nodeId, outgoing);
  }
  return new Set();
}
