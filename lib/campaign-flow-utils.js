...
export function getFlowNodeKind(node, taskById) {
  ...
  if (data.nodeType === 'frame' || node.nodeType === 'frame') return 'frame';
  if (data.nodeType === 'label' || node.nodeType === 'label') return 'label';
...
export function normalizeFlowEdgeDirection(edge, nodeById, taskById) {
  ...
  if (sourceKind === 'label' || sourceKind === 'frame' || targetKind === 'label' || targetKind === 'frame') {
    return edge;
  }
...
export function flowStructureKey(flowData, boards = []) {
  ...
  const nodeKey = nodes
    .map(node => `${node.id}:${node.taskId || ''}:${node.boardId || ''}:${node.nodeType || ''}:${node.notes || ''}`)
    .sort()
    .join(',');
  const edgeKey = edges
    .map(edge =>
 ...
 * Root = where arrows point TO (incoming / sink) — upstream nodes follow when you drag it.
 * Source-only nodes (arrows go FROM) drag alone; downstream is not pulled.
