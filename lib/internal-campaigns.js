...
export function defaultLabelForStatusId(id) {
  const key = KNOWN_STATUS_LABEL_KEYS[id];
  if (key) return key;
  return String(id || '')
    .replace(/_/g, ' ')
...
export function statusColumnLabel(column, t) {
  ...
  const customLabel = typeof column === 'object' ? column?.label : '';
  const key = KNOWN_STATUS_LABEL_KEYS[id];
  if (key && t) {
  ...
  return defaultLabelForStatusId(id);
}
...
export function boardStatusColumns(board) {
  if (board?.status_columns?.length) return normalizeStatusColumns(board.status_columns);
  return BOARD_STATUSES.map(id => normalizeStatusColumn(id));
}
