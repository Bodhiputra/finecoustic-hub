...
import { normalizeFlowEdgeDirection, sanitizeFlowEdgeHandle } from '@/lib/campaign-flow-utils';
import { isMarketingKolOutreachScope } from '@/lib/campaign-kol';
import { isFrameworkFlowNodeType, normalizeMapMode } from '@/lib/framework-map';
    ...
    ? data.nodes
        .map(node => {
          ...
          if (isFrameworkFlowNodeType(node?.nodeType)) {
            const nodeType = node.nodeType === 'frame' ? 'frame' : 'label';
            return {
              id,
              nodeType,
              taskId: null,
              boardId: null,
              label: String(node?.label || '').trim().slice(0, 120) || 'Untitled',
              notes: nodeType === 'frame' ? String(node?.notes || '').trim().slice(0, 500) : '',
              position: {
  ...
  const edges = Array.isArray(data.edges)
    ? data.edges
        .map(edge => {
          ...
          const sourceHandle = sanitizeFlowEdgeHandle(edge?.sourceHandle, 'source');
          const targetHandle = sanitizeFlowEdgeHandle(edge?.targetHandle, 'target');
          return {
            id,
            source,
            target,
...
/**
 * Order flow tasks for board/list projections — sink-first (where arrows point TO),
 * then walk upstream; tie-break by canvas Y then X. Tasks without flow nodes sort last.
 */
export function sortTasksByFlowOrder(tasks, flowData) {
  ...
  for (const edge of flow.edges) {
    ...
    predecessors.get(toTask).push(fromTask);
    outDegree.set(fromTask, (outDegree.get(fromTask) || 0) + 1);
  }

  const queue = list
    .filter(task => (outDegree.get(task.id) || 0) === 0)
    .map(task => task.id)
  ...
  while (queue.length) {
    ...
    for (const prevId of predecessors.get(id) || []) {
      outDegree.set(prevId, (outDegree.get(prevId) || 0) - 1);
      if (outDegree.get(prevId) === 0) {
        queue.push(prevId);
...
export function normalizeCampaign(raw) {
  ...
  return {
    ...
    map_mode: normalizeMapMode(c.map_mode),
...
export function normalizeBoard(raw) {
  ...
  return {
    ...
    status_columns: isMarketingKolOutreachScope(id)
      ? normalizeStatusColumns(b.status_columns)
      : ensureWorkflowStatusColumns(b.status_columns),
