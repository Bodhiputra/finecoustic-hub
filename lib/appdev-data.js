...
import { emptyBoard, normalizeBoard, STATUSES, PRIORITIES, touchPeople, formatIssueId, normalizeComment, parseIssueDatePatch, mergePeople, personKey, normalizeIssueType, mergeTaskTypes } from './appdev';
...
export async function createIssue(input, actor = null) {
  ...
  await saveAppdevData(board);
  return { issue, people, task_types, next_number: board.next_number };
}
...
export async function updateIssue(id, patch, options = {}) {
  ...
  await saveAppdevData(board);
  return { issue: next, people, task_types };
}
