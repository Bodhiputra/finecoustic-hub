...
import { emptyBoard, normalizeBoard, STATUSES, PRIORITIES, ISSUE_TYPES, touchPeople, formatIssueId, normalizeComment, parseIssueDatePatch, mergePeople } from './appdev';
...
async function mergeRegisteredPeopleList(existing = []) {
  const { listAssignablePeople } = await import('./appdev-users.js');
  return mergePeople(existing, await listAssignablePeople());
}
