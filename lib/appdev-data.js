...
async function enrichBoardForRead(board, { includeAssignable = true } = {}) {
  ...
  } else if (!Array.isArray(board.meta?.assignable_people)) {
    board.meta.assignable_people = board.meta?.people || [];
  }
  return board;
}
...
export async function createIssue(input, actor = null) {
  ...
  if (useRelationalStorage()) {
    const num = await getRelationalNextNumber();
    ...
    const inserted = await insertRelationalIssue(issue, { nextNumber: num + 1 });
