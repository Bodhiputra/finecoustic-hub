...
async function ensureTables() {
  ...
  if (!tablesReadyPromise) {
    ...
    `
      ...
      `)
      .then(() => sql()`ALTER TABLE hub_product_items ADD COLUMN IF NOT EXISTS reporter_type TEXT NOT NULL DEFAULT 'end_user'`)
      .then(() => sql()`ALTER TABLE hub_product_items ADD COLUMN IF NOT EXISTS platform TEXT NOT NULL DEFAULT ''`)
      .then(() => sql()`ALTER TABLE hub_product_items ADD COLUMN IF NOT EXISTS correspondent TEXT NOT NULL DEFAULT ''`)
      .then(() => sql()`ALTER TABLE hub_product_items ADD COLUMN IF NOT EXISTS suggested_by TEXT NOT NULL DEFAULT ''`)
      .then(() => {
        tablesReady = true;
      })
      .catch(err => {
...
async function readAllItemsRaw() {
  if (useDatabase()) {
    await ensureTables();
    const rows = await sql()`
      SELECT id, product_sku, kind, title, body, source, reporter_type, platform,
             correspondent, suggested_by, status, assignee, source_ref,
  ...
  if (useDatabase()) {
    ...
    await sql()`
      INSERT INTO hub_product_items (
        id, product_sku, kind, title, body, source, reporter_type, platform,
        correspondent, suggested_by, status, assignee, source_ref,
        comments, created_by, created_at, updated_at
      ) VALUES (
        ${normalized.id}, ${normalized.product_sku}, ${normalized.kind}, ${normalized.title},
        ${normalized.body}, ${normalized.source}, ${normalized.reporter_type}, ${normalized.platform},
        ${normalized.correspondent}, ${normalized.suggested_by}, ${normalized.status}, ${normalized.assignee},
...
function catalogEntryNeedsFill(existing, entry) {
  if (entry.name && String(existing.name || '').trim() !== String(entry.name).trim()) return true;
...
function buildCatalogPatch(existing, entry, fillMissing) {
  ...
  const patch = {};
  if (entry.name && String(existing.name || '').trim() !== String(entry.name).trim()) {
