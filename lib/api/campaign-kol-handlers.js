...
async function resolveKolPipelineScope(campaignId) {
  if (isMarketingKolOutreachScope(campaignId)) {
    return { id: KOL_OUTREACH_BOARD_ID, name: 'KOL outreach' };
  }
...
export async function getCampaignKol(_request, { params }) {
  ...
  const campaign = await resolveKolPipelineScope(campaignId);
