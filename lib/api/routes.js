...
export const API_V1 = {
  ...
  internalTasksRevision: '/api/v1/internal/tasks/revision',
  ...
  hubNotificationsPing: '/api/v1/hub/notifications?ping=1',
  ...
  marketingKolTrackingCodes: '/api/v1/marketing/kol-tracking-codes',
  marketingKolTrackingCode: id => `/api/v1/marketing/kol-tracking-codes/${encodeURIComponent(id)}`,
  ...
  opsB2bRetailers: '/api/v1/ops/b2b-retailers',
  opsB2bRetailerRecord: id => `/api/v1/ops/b2b-retailers/${encodeURIComponent(id)}`,
...
export function internalTasksRevisionQuery({ department, board_id, campaign_id, flow_only, hub_home } = {}) {
  const params = new URLSearchParams();
  if (department) params.set('department', department);
  ...
  if (hub_home) params.set('hub_home', '1');
  const qs = params.toString();
  return qs ? `${API_V1.internalTasksRevision}?${qs}` : API_V1.internalTasksRevision;
}

export function marketingKolPoolQuery({
  section = 'masterlist',
  limit = 25,
  offset = 0,
  q = '',
  ...
  if (poolView) params.set('pool_view', poolView);
  if (!metaOnly && !poolView) {
    ...
    if (collabInitiative && collabInitiative !== 'all') {
      params.set('collab_initiative', collabInitiative);
    }
  }
  const qs = params.toString();
  return qs ? `${API_V1.marketingKolPool}?${qs}` : API_V1.marketingKolPool;
}
...
export function internalBoardsQuery({ department, scope, forFlowPicker = false } = {}) {
  const params = new URLSearchParams();
  if (department) params.set('department', department);
  if (scope) params.set('scope', scope);
  if (forFlowPicker) params.set('for_flow_picker', '1');
  const qs = params.toString();
