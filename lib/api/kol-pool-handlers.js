...
import { countKolBySection, filterVisibleKolPool } from '@/lib/kol-pool';
import { personKey } from '@/lib/appdev';
import { createHubNotification } from '@/lib/hub-notifications';
import { queryKolPoolPage } from '@/lib/kol-pool-query';
import {
  isKolPoolConfigured,
  listKolPoolOutreachRecords,
  listKolPoolRecords,
  readKolPoolMetaOnly,
...
export async function getKolPool(request) {
  ...
  const poolView = searchParams.get('pool_view') || '';

  if (metaOnly) {
    const meta = await readKolPoolMetaOnly();
    return restOk({
      meta,
  ...
  if (poolView === 'outreach') {
    const records = filterVisibleKolPool(await listKolPoolOutreachRecords());
    const meta = await readKolPoolMetaOnly();
    return restOk({
      records,
      meta,
      counts: countKolBySection(records),
  ...
  const collabInitiative = searchParams.get('collab_initiative') || 'all';

  const page = await queryKolPoolPage({
    section,
    limit,
    offset,
    q,
  ...
  return restOk({
    ...page,
    configured: isKolPoolConfigured(),
    total_visible: page.counts?.masterlist ?? page.total,
  });
}

/** Manual Notion sync only — triggered by POST /api/v1/marketing/kol-pool/sync (UI button). */
export async function postKolPoolSync(scheduleAfter) {
  ...
  try {
    await runSync();
    const nextMeta = await readKolPoolMetaOnly();
...
export async function postKolPool(request) {
  ...
  try {
    ...
    if (e.status === 409 && e.message === 'channel_name_duplicate') {
      return restError('channel_name_duplicate', 409);
    }
    if (e.status === 400) {
...
/** Server layout seed — meta only; records load via GET /api/v1/marketing/kol-pool. */
...
export async function loadKolPoolOutreachForPage() {
  const records = filterVisibleKolPool(await listKolPoolOutreachRecords());
  const meta = await readKolPoolMetaOnly();
  return {
    records,
...
export async function getKolPoolRecordHandler(_request, { params }) {
  let actor;
  try {
    actor = await requireHubActor();
  } catch {
  ...
  const record = await getKolPoolRecord(notionPageId);
  if (!record) return restError('not_found', 404);
  return restOk({ record });
}
...
export async function deleteKolPoolRecordHandler(_request, { params }) {
  let actor;
  try {
    actor = await requireHubActor();
  } catch {
  ...
  try {
    const deleted = await deleteKolPoolRecord(notionPageId);
