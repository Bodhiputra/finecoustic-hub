...
export async function postKolPoolSync() {
  let actor;
  try {
    actor = await requireHubActor();
  } catch {
  ...
  try {
    const { records, meta } = await syncKolPoolFromNotion(actor.displayName);
    const visible = filterVisibleKolPool(records);
    const counts = countKolBySection(visible);
    ...
    await createHubNotification({
      ...
      dedupeKey: `kol_sync:${personKey(actor.displayName)}:${meta.last_synced_at}`,
    });
