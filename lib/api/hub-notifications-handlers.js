...
import { dismissAllStaleKolOutreachNotifications, processKolOutreachAlerts } from '@/lib/kol-outreach-alerts';
...
let lastKolSweepOnPing = 0;
const KOL_SWEEP_PING_THROTTLE_MS = 10 * 60 * 1000;

async function sweepStaleKolOutreachNotifications() {
  try {
    const tasks = await listKolOutreachTasksForAlerts();
    await dismissAllStaleKolOutreachNotifications(tasks);
  } catch (err) {
    console.error('[sweepStaleKolOutreachNotifications]', err);
  }
}
...
export async function getHubNotifications(request) {
  ...
  try {
    if (ping) {
      const now = Date.now();
      if (now - lastKolSweepOnPing >= KOL_SWEEP_PING_THROTTLE_MS) {
        lastKolSweepOnPing = now;
        await sweepStaleKolOutreachNotifications();
      }
