...
/** KOL pool / outreach product chips — marketing may read catalog without Products dept. */
function requireProductsListAccess(actor) {
  if (actor.mustChangePassword) return restForbidden('must_change_password');
  if (canAccessDepartment(actor, 'products') || canAccessDepartment(actor, 'marketing')) {
    return null;
  }
...
export async function listProducts(_request) {
  ...
  const denied = requireProductsListAccess(actor);
