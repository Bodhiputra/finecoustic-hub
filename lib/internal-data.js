...
import { resolveWorkflowPatch, applyAutoStartWhenAssigned } from '@/lib/task-workflow';
...
export async function listTasksForActor(actor, {
  ...
  items = items.map(t => applyAutoStartWhenAssigned(t));
...
export async function getTaskById(id, actor) {
  ...
  const fixed = applyAutoStartWhenAssigned(task);
  if (fixed.status !== task.status) {
    return writeOne({ ...fixed, updated_at: new Date().toISOString() });
  }
  return task;
}
