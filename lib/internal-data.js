...
import {
  ...
  isUndatedTask,
  isScheduledTask,
  isHubHomeScheduleTask,
  TEAM_SCHEDULE_KINDS,
  todayKey,
} from '@/lib/internal';
import { resolveWorkflowPatch, applyAutoStartWhenAssigned, taskAssigneeKey } from '@/lib/task-workflow';
import { syncTaskAssigneeFields } from '@/lib/task-assignees';
...
export async function listKolOutreachTasksForAlerts() {
  ...
  return items.filter(task => task?.kind !== 'meeting');
}
...
/** Hub home calendar — team tasks, milestones, and meetings (not department board tasks). */
async function readTasksForHubHome() {
  const scheduleKinds = new Set(TEAM_SCHEDULE_KINDS);
  if (!useDatabase()) {
    return readFileStore().filter(
      t => t.status !== 'archived'
        && t.status !== 'cancelled'
        && (scheduleKinds.has(t.kind) || isHubHomeScheduleTask(t))
    );
  }
  await ensureTable();
  const rows = await withNeonRetry(() => sql()`
    ...
    WHERE status NOT IN ('archived', 'cancelled')
      AND (
        COALESCE(data->>'kind', 'task') IN ('milestone', 'meeting', 'event')
        OR (
          COALESCE(data->>'kind', 'task') = 'task'
          AND COALESCE(data->>'board_id', '') = ''
          AND COALESCE(data->>'campaign_id', '') = ''
          AND LOWER(COALESCE(data->>'department', '')) = 'all'
        )
      )
...
/** Lightweight change detection for workspace polling — no full task payloads. */
export async function getInternalTasksRevision({
  department,
  board_id,
  campaign_id,
  flow_only,
  ...
  if (!useDatabase()) {
    const items = await (async () => {
      if (board_id) return readTasksScoped({ board_id });
      if (campaign_id) return readTasksScoped({ campaign_id, flow_only });
      if (department) return readTasksScoped({ department });
      if (hub_home) return readTasksForHubHome();
      if (assigned_to_me) return readTasksAssignedTo({ displayName: '' });
      return readAllRaw();
    })();
    const max = items.reduce((acc, t) => {
      const u = String(t?.updated_at || '');
      return u > acc ? u : acc;
    }, '');
    return { updated_at: max || null, count: items.length, etag: `${max}|${items.length}` };
  }
  ...
  if (board_id) {
    ...
    `);
    const row = rows[0] || {};
    const etag = `${row.updated_at || ''}|${row.count || 0}`;
    return { updated_at: row.updated_at, count: row.count, etag };
  }
  ...
  if (campaign_id) {
    if (flow_only) {
      ...
      `);
      const row = rows[0] || {};
      return { updated_at: row.updated_at, count: row.count, etag: `${row.updated_at || ''}|${row.count || 0}` };
    }
    const rows = await withNeonRetry(() => sql()`
      SELECT COUNT(*)::int AS count, MAX(updated_at) AS updated_at
      FROM internal_tasks
  ...
  const rows = await withNeonRetry(() => sql()`
    SELECT COUNT(*)::int AS count, MAX(updated_at) AS updated_at FROM internal_tasks
  `);
  const row = rows[0] || {};
  return { updated_at: row.updated_at, count: row.count, etag: `${row.updated_at || ''}|${row.count || 0}` };
}
    ...
    WHERE assignee_key = ${key}
       OR EXISTS (
         ...
         FROM jsonb_array_elements_text(
           CASE
             WHEN jsonb_typeof(data->'assignees') = 'array' THEN data->'assignees'
             ELSE '[]'::jsonb
           END
         ) AS elem(val)
         WHERE lower(trim(elem.val)) = ${key}
       )
       OR (
         COALESCE(assignee_key, '') = ''
         AND NOT (
           CASE
             WHEN jsonb_typeof(data->'assignees') = 'array' THEN jsonb_array_length(data->'assignees')
             ELSE 0
           END
         ) > 0
...
async function writeOne(task) {
  const normalized = syncTaskAssigneeFields(normalizeTask(task));
  if (useDatabase()) {
    await ensureTable();
...
export async function updateTask(id, patch, actor) {
  ...
  // Legacy clients send `assignee` only; keep `assignees` in sync before merge.
  if (safePatch.assignee !== undefined && safePatch.assignees === undefined) {
    const name = String(safePatch.assignee || '').trim();
    safePatch.assignees = name ? [name] : [];
  }
  ...
  if (String(existing.board_id || '') === KOL_OUTREACH_BOARD_ID && next.kind !== 'meeting') {
    ...
    if (nextOrder && nextOrder !== prevOrder) {
      ...
      const result = validateKolOrderNumber(nextOrder, tasks, next.id);
      if (!result.ok) {
        const err = new Error(
          result.code === 'duplicate' ? 'kol_order_number_duplicate' : 'kol_order_number_invalid'
        );
        err.status = result.code === 'duplicate' ? 409 : 400;
        err.detail = result.conflict?.title || '';
        throw err;
      }
      next.custom_values = {
        ...(next.custom_values || {}),
