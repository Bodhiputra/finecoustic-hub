...
import { canCreateTask, canCreateTaskInDepartment, canDeleteTask } from '@/lib/hub-permissions';
import { getBoardById } from '@/lib/internal-campaigns-data';
import { runTaskCommentSideEffects, runTaskUpdateSideEffects, notifyTaskAssigned } from '@/lib/internal-task-notifications';
...
/** All tasks for cron schedule alerts (deadlines + meetings). */
...
/** Hub home calendar — skip archived/cancelled rows at the DB instead of loading everything. */
async function readTasksForHubHome() {
  if (!useDatabase()) {
    return readFileStore().filter(t => t.status !== 'archived' && t.status !== 'cancelled');
  }
  await ensureTable();
...
export async function createTask(input, actor) {
  ...
  if (!canCreateTaskInDepartment(actor, deptForPermission)) {
    const err = new Error('forbidden');
    err.status = 403;
    throw err;
  }
  ...
  const saved = await writeOne(task);
...
export async function addTaskComment(id, input, actor) {
  ...
  const saved = await writeOne(next);
  const knownNames = await listActiveTeamMemberNames().catch(() => []);
