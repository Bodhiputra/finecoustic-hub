...
/** Names that appear as assignees on at least one issue (optionally limited to registered users). */
export function assigneeFilterOptions(issues, registeredPeople = null) {
  const allowed =
    registeredPeople != null
      ? new Set(registeredPeople.map(name => personKey(name)))
      : null;
  const map = new Map();
