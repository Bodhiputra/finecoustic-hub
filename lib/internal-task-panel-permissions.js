...
import { isHubManager, isHubAdmin, isHubIntern } from '@/lib/hub-permissions';
import {
  isKolOutreachTask,
  isTaskAssignee,
  isTaskAssigner,
...
import { getTaskAssignees, hasTaskAssignees } from '@/lib/task-assignees';

/** Unassigned KOL outreach cards — marketing team can claim by assigning themselves. */
export function canSelfAssignKolOutreachCard(task, actor = {}) {
  if (!task?.id || !isKolOutreachTask(task)) return false;
  if (!isTaskUnassigned(task)) return false;
  if (!normStr(actor.displayName) || actor.mustChangePassword) return false;
  if (isHubIntern(actor)) return false;
  return canAccessDepartment(actor, 'marketing');
}

/** Marketing can assign any teammate on KOL outreach cards (not only self-claim). */
export function canAssignKolOutreachTeam(task, actor = {}) {
  if (!task?.id || !isKolOutreachTask(task)) return false;
  if (!normStr(actor.displayName) || actor.mustChangePassword) return false;
  if (isHubIntern(actor)) return false;
  return canAccessDepartment(actor, 'marketing');
...
export function getTaskPanelCapabilities(task, actor = {}) {
  ...
  const unassignedWorkflow = !isNew && task?.kind === 'task' && !hasTaskAssignees(task);
  const canEditMetadata = isNew || assigner || manager || admin;
  const kolOutreach = !isNew && isKolOutreachTask(task);
  const canSelfAssignKol = canSelfAssignKolOutreachCard(task, actor);
  const canAssignKolTeam = canAssignKolOutreachTeam(task, actor);
  const canEditKolFields =
    kolOutreach && (assignee || assigner || manager || admin || canSelfAssignKol || canAssignKolTeam);
  ...
  return {
    ...
    canEditKolFields,
    canSelfAssignKol,
    canAssignKolOutreachTeam: canAssignKolTeam,
    canManageAttachments: canEditMetadata,
    canEditAssignee: canEditMetadata || canAssignKolTeam,
    canEditCalendarFields: canEditMetadata,
    canManageCustomFields: canEditMetadata || (kolOutreach && (assignee || canSelfAssignKol)),
...
export function sanitizeTaskPatch(existing, patch, actor) {
  ...
  for (const key of METADATA_KEYS) {
    ...
    if (kolFieldsOnly && (key === 'assignee' || key === 'assignees')) {
      if (next.assignee !== undefined || next.assignees !== undefined) {
        const existingAssignees = getTaskAssignees(existing);
        const nextAssignees = getTaskAssignees({
          assignees: next.assignees,
          assignee: next.assignee ?? existing.assignee,
        });
        const actorKey = personKey(actor.displayName);
        const selfAssign =
          canSelfAssignKolOutreachCard(existing, actor)
          && nextAssignees.length === 1
          && personKey(nextAssignees[0]) === actorKey
          && !existingAssignees.length;
        const teamAssign =
          canAssignKolOutreachTeam(existing, actor)
          && nextAssignees.length === 1
          && normStr(nextAssignees[0]);
        if (!selfAssign && !teamAssign) {
          delete next.assignee;
          delete next.assignees;
        } else {
          next.assignees = nextAssignees;
          next.assignee = nextAssignees[0] || '';
        }
      }
      continue;
    }
