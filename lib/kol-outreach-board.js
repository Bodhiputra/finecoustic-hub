...
import { getBoardById, updateBoard, writeBoardSeed } from '@/lib/internal-campaigns-data';
...
import { migrateKolOutreachLegacyStatuses } from '@/lib/kol-outreach-alerts';
...
export async function ensureKolOutreachBoard(actor = { displayName: 'System' }) {
  ...
  const desiredColumns = defaultKolOutreachStatusColumns();
  const desiredProps = defaultKolOutreachCustomProperties();

  if (existing) {
    const colsStale =
      JSON.stringify(existing.status_columns || []) !== JSON.stringify(desiredColumns);
    const propsStale =
      JSON.stringify(existing.custom_properties || []) !== JSON.stringify(desiredProps);
    if (colsStale || propsStale) {
      await updateBoard(KOL_OUTREACH_BOARD_ID, {
        status_columns: desiredColumns,
        custom_properties: desiredProps,
        updated_at: now,
      });
    }
    await migrateKolOutreachLegacyStatuses().catch(() => {});
    return getBoardById(KOL_OUTREACH_BOARD_ID);
  }
...
function entryToTask(entry, boardId, actorName) {
  ...
  return {
    ...
    department: 'marketing',
    subtype: 'kol',
    status: normalizeKolOutreachStatus(entry.pipeline_status || 'not_started'),
    board_id: boardId,
    custom_values: {
