...
export const KOL_TAG_SUGGESTIONS = ['stored', 'qualified', 'unqualified', 'needs confirmation'];

/** Follower-tier options for kol_category — ids are stored values. */
export const KOL_TIER_OPTIONS = [
  { id: 'Nano (<10K)', labelKey: 'hub.kol.tierNano' },
  { id: 'Micro (10K-100K)', labelKey: 'hub.kol.tierMicro' },
  { id: 'Mid-tier (100K-500K)', labelKey: 'hub.kol.tierMid' },
  { id: 'Macro (500K-1M)', labelKey: 'hub.kol.tierMacro' },
  { id: 'Mega (1M+)', labelKey: 'hub.kol.tierMega' },
];

export const KOL_TAG_LABEL_KEYS = {
  stored: 'hub.kol.stored',
  qualified: 'hub.kol.qualified',
  unqualified: 'hub.kol.unqualified',
  'needs confirmation': 'hub.kol.needsConfirmation',
};

export function normalizeKolTagChoice(raw = '', { isCreate = false } = {}) {
  const text = String(raw || '').trim().toLowerCase();
  if (/\bunqualified\b/.test(text) || /\bdisqualified\b/.test(text)) return 'unqualified';
  if (/\bqualified\b/.test(text)) return 'qualified';
  if (/\bneeds confirmation\b/.test(text) || /\bneed confirmation\b/.test(text)) return 'needs confirmation';
  if (/\bstored\b/.test(text)) return 'stored';
  return isCreate ? 'stored' : '';
}

export function normalizeKolTierChoice(raw = '') {
  const text = String(raw || '').trim().toLowerCase();
  if (!text) return '';
  if (text.includes('nano') || text.includes('<10')) return 'Nano (<10K)';
  if (text.includes('micro') || (text.includes('10k') && text.includes('100k'))) return 'Micro (10K-100K)';
  if (text.includes('mid') || text.includes('100k-500k') || text.includes('100k–500k')) return 'Mid-tier (100K-500K)';
  if (text.includes('macro') || text.includes('500k-1m') || text.includes('500k–1m')) return 'Macro (500K-1M)';
  if (text.includes('mega') || text.includes('1m+') || text.includes('1m +')) return 'Mega (1M+)';
  const exact = KOL_TIER_OPTIONS.find(option => option.id.toLowerCase() === text);
  return exact?.id || String(raw || '').trim();
}
...
export function collabEntryMatchesProduct(entry, product) {
  const text = String(entry || '').trim().toLowerCase();
  if (!text || !product) return false;
  const sku = String(product.sku || '').trim().toLowerCase();
  const name = String(product.name || '').trim().toLowerCase();
  return text === sku || (sku && text.includes(sku)) || (name && text.includes(name));
}

/** Split stored collaboration strings into catalog SKUs vs legacy free-text entries. */
export function partitionCollabProductsByCatalog(entries, catalog = []) {
  const legacy = [];
  const skus = new Set();
  for (const entry of normalizeCollaborationProducts(entries)) {
    const match = catalog.find(product => collabEntryMatchesProduct(entry, product));
    if (match) skus.add(match.sku);
    else legacy.push(entry);
  }
  return { skus: [...skus], legacy };
}
...
export function normalizeKolPoolRecord(raw = {}) {
  return {
    ...
    shipping_name: String(raw.shipping_name || '').trim(),
    ...
    shipping_country_code: normalizeShippingCountryCode(raw.shipping_country_code),
    shipping_phone: String(raw.shipping_phone || '').trim(),
    shipping_email: String(raw.shipping_email || '').trim(),
    shipping_tax_id: String(raw.shipping_tax_id || '').trim(),
...
export function kolShippingRecipientName(record = {}, fallback = '') {
  return String(record.shipping_name || fallback || record.channel_name || '').trim();
}

/** Default Weibin Excel 备注 when delivery notes are empty. */
export const KOL_WEIBIN_DEFAULT_DELIVERY_NOTE = '需要检测';

/** Strip legacy mistaken “Recipient: …” lines that duplicate shipping_name. */
export function normalizeKolDeliveryNotesRaw(record = {}) {
  let notes = String(record?.shipping_notes || '').trim();
  if (!notes || !/^recipient\s*:/i.test(notes)) return notes;
  const extracted = notes.replace(/^recipient\s*:\s*/i, '').trim();
  const name = String(record?.shipping_name || '').trim();
  if (!extracted) return '';
  if (!name || extracted.localeCompare(name, undefined, { sensitivity: 'accent' }) === 0) {
    return '';
  }
  return notes;
}

export function kolDeliveryNotesDisplay(record = {}) {
  const notes = normalizeKolDeliveryNotesRaw(record);
  return notes || KOL_WEIBIN_DEFAULT_DELIVERY_NOTE;
}

export function normalizeShippingCountryCode(raw = '') {
  return String(raw || '').trim().toUpperCase();
}

export function isValidShippingCountryCode(raw = '') {
  return /^[A-Z]{2}$/.test(normalizeShippingCountryCode(raw));
}

/** ISO 2-letter code for Weibin export and carriers — prefers shipping_country_code. */
export function kolShippingCountryCode(record = {}) {
  const code = normalizeShippingCountryCode(record.shipping_country_code);
  if (isValidShippingCountryCode(code)) return code;
  const legacy = normalizeShippingCountryCode(record.shipping_country);
  if (isValidShippingCountryCode(legacy)) return legacy;
  return '';
}
...
export function kolShippingSummary(record) {
  ...
  const countryLabel = [
    ...
    kolShippingCountryCode(record) ? `(${kolShippingCountryCode(record)})` : '',
  ].filter(Boolean).join(' ');
  const parts = [
...
export const KOL_SHIPPING_DETAIL_FIELDS = [
  { key: 'shipping_name', labelKey: 'hub.kol.shippingName' },
  ...
  { key: 'shipping_country_code', labelKey: 'hub.kol.shippingCountryCode' },
  { key: 'shipping_phone', labelKey: 'hub.kol.shippingPhone' },
  { key: 'shipping_email', labelKey: 'hub.kol.shippingEmail' },
  { key: 'shipping_tax_id', labelKey: 'hub.kol.shippingTaxId' },
...
/** English labels for pasting into carriers / Weibin — not locale-dependent. */
export const KOL_SHIPPING_CLIPBOARD_LINES = [
  { key: 'shipping_name', label: "Recipient's Full Name" },
  { key: '_street', label: 'Street Address' },
  { key: 'shipping_city', label: 'City' },
  { key: 'shipping_postal', label: 'Postcode' },
  { key: 'shipping_state', label: 'Province' },
  { key: 'shipping_country', label: 'Country' },
  { key: 'shipping_email', label: 'Email Address' },
  { key: 'shipping_phone', label: 'Phone Number' },
  {
    key: 'shipping_tax_id',
    label: 'Tax/VAT/EORI Number (only if required for customs in your country)',
  },
];

export function kolShippingStreetLine(record = {}) {
  return [record.shipping_line1, record.shipping_line2]
    .map(s => String(s || '').trim())
    .filter(Boolean)
    .join(' ');
}

export function kolShippingClipboardValue(record, key) {
  if (key === '_street') return kolShippingStreetLine(record);
  return String(record[key] || '').trim();
}

/** Plain-text block for clipboard (English field labels). Omits empty lines. */
export function kolShippingClipboardText(record) {
  if (!record) return '';
  const lines = [];
  for (const { key, label } of KOL_SHIPPING_CLIPBOARD_LINES) {
    const value = kolShippingClipboardValue(record, key);
    if (value) lines.push(`${label}:  ${value}`);
  }
  return lines.join('\n');
}

export function hasKolShippingClipboard(record) {
  return Boolean(kolShippingClipboardText(record));
}

export function hasKolShippingAddress(record) {
  if (!record) return false;
  return Boolean(
    String(record.shipping_name || '').trim()
    && String(record.shipping_line1 || '').trim()
    && String(record.shipping_city || '').trim()
    && kolShippingCountryCode(record)
  );
}
...
/** Icon name for outreach cards — pool link/platform first, then approached socials on the card. */
export function kolOutreachPlatformIconName(poolRecord, approachedSocials = '') {
  const poolIcon = kolLinkIconName(poolRecord || {});
  if (poolIcon !== 'externalLink') return poolIcon;

  for (const part of String(approachedSocials).split(',')) {
    const trimmed = part.trim();
    if (!trimmed || trimmed.toLowerCase() === 'email') continue;
    const icon = kolLinkIconName({ main_platform: trimmed, links: '' });
    if (icon !== 'externalLink') return icon;
  }
  return null;
}
...
function hasStoredTag(record) {
  return /\bstored\b/.test(tagText(record));
}

function hasQualifiedTag(record) {
  return /\bqualified\b/.test(tagText(record));
}
...
/** Normalize channel handle for duplicate detection (@, case, whitespace). */
export function kolChannelDedupeKey(channelName = '') {
...
function kolPoolRecordDedupeScore(record) {
  if (!record) return 0;
  let score = 0;
  if (!isHubNativeKol(record)) score += 16;
  if (record.notion_url) score += 8;
  if (record.links) score += 4;
  if (hasKolShippingAddress(record)) score += 4;
  if (record.description) score += 2;
  if (record.main_platform) score += 1;
  if (record.country) score += 1;
  if (record.synced_at) score += 0.001 * Date.parse(record.synced_at);
  return score;
}

/** Keep one record per channel — prefer Notion over hub-native, richer profile wins. */
export function dedupeKolPoolRecords(records) {
  const list = (Array.isArray(records) ? records : []).map(r => applyKolCoreOverrides(r));
  const byChannel = new Map();

  for (const record of list) {
    const key = kolChannelDedupeKey(record.channel_name);
    const mapKey = key || `id:${record.notion_page_id}`;
    const prev = byChannel.get(mapKey);
    if (!prev || kolPoolRecordDedupeScore(record) > kolPoolRecordDedupeScore(prev)) {
      byChannel.set(mapKey, record);
    }
  }

  return [...byChannel.values()].sort((a, b) =>
    String(a.channel_name || '').localeCompare(String(b.channel_name || ''), undefined, { sensitivity: 'base' })
  );
}

export function filterVisibleKolPool(records) {
  return dedupeKolPoolRecords((Array.isArray(records) ? records : []).filter(isKolVisibleInPool));
}

/** Outreach "Add from pool" — stored-only KOLs are not outreach-ready. */
export function isKolSelectableForOutreach(record) {
  if (!isKolVisibleInPool(record)) return false;
  if (hasQualifiedTag(record)) return true;
  if (hasStoredTag(record)) return false;
  return true;
}

export function filterKolPoolForOutreach(records) {
  return dedupeKolPoolRecords(
    (Array.isArray(records) ? records : []).filter(isKolSelectableForOutreach)
  );
}
...
export function pickKolHubPatch(body = {}) {
  ...
  for (const key of KOL_HUB_FIELD_KEYS) {
    ...
    } else if (key === 'shipping_country_code') {
      patch[key] = normalizeShippingCountryCode(body[key]);
    } else {
      patch[key] = String(body[key] ?? '').trim();
    }
  }
