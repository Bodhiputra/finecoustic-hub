...
let assignableCache = { at: 0, names: null };

const ASSIGNABLE_CACHE_MS = 60_000;

export function invalidateAssignablePeopleCache() {
  assignableCache = { at: 0, names: null };
}

export async function listAssignablePeople() {
  const now = Date.now();
  if (assignableCache.names && now - assignableCache.at < ASSIGNABLE_CACHE_MS) {
    return assignableCache.names;
  }
  const users = await listUsers();
  const names = users.filter(u => !u.blocked).map(u => u.display_name);
  assignableCache = { at: now, names };
  return names;
}
...
export async function createUser(displayName) {
  ...
  invalidateAssignablePeopleCache();
...
export async function deleteUser(id) {
  if (useDatabase()) {
    ...
    if (rows.length > 0) invalidateAssignablePeopleCache();
