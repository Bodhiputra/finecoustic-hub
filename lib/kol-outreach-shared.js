...
/** Soft normalize — canonical `KOL BP{n}` when input matches, otherwise keep trimmed text. */
export function validateKolOrderNumber(raw) {
  const text = String(raw || '').trim();
  if (!text) return { ok: true, normalized: '' };
  const normalized = normalizeKolOrderNumber(raw);
  return { ok: true, normalized: normalized || text };
}
