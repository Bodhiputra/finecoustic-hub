...
export const KOL_BOARD_PROP = {
  ...
  weibinBatchCode: 'prop_weibin_batch_code',
...
export function defaultKolOutreachCustomProperties() {
  return [
    ...
    { id: KOL_BOARD_PROP.weibinBatchCode, type: 'text', label: 'Weibin batch code' },
...
/** Suggest batch label for Weibin export, e.g. KOL BP20 */
export function suggestWeibinBatchCode(task, poolRecord) {
  const existing = String(task?.custom_values?.[KOL_BOARD_PROP.weibinBatchCode] || '').trim();
  if (existing) return existing;
  const channel = String(poolRecord?.channel_name || task?.title || '').trim();
  const slug = channel.replace(/^@/, '').replace(/\s+/g, '').slice(0, 12);
  return slug ? `KOL ${slug}` : '';
}

export function weibinExportFilename(batchCodes = []) {
  const codes = [...new Set(batchCodes.map(c => String(c || '').trim()).filter(Boolean))].sort();
