...
/** Highest sequence assigned outside the outreach board (BP01–BP20 pre-registry). */
export const KOL_ORDER_NUMBER_EXTERNAL_FLOOR = 20;
...
/** Normalize user input to canonical `KOL BP{n}` or empty when invalid. */
export function normalizeKolOrderNumber(raw = '') {
  const sequence = parseKolOrderNumberSequence(raw);
  return sequence ? formatKolOrderNumber(sequence) : '';
}

export function latestKolOrderNumberSequence(tasks = []) {
  let max = KOL_ORDER_NUMBER_EXTERNAL_FLOOR;
  for (const task of Array.isArray(tasks) ? tasks : []) {
    const sequence = parseKolOrderNumberSequence(kolOutreachOrderNumber(task?.custom_values));
...
export function kolOrderNumberUsageBySequence(tasks = [], { excludeTaskId = null } = {}) {
  ...
  for (const task of Array.isArray(tasks) ? tasks : []) {
    ...
    usage.set(sequence, {
      taskId: task.id,
      title: task.title || '',
      formatted: formatKolOrderNumber(sequence),
    });
  }
  return usage;
}

function kolOrderNumberHeldByTask(tasks, taskId) {
  if (!taskId) return null;
  const task = (Array.isArray(tasks) ? tasks : []).find(item => item?.id === taskId);
  if (!task) return null;
  return parseKolOrderNumberSequence(kolOutreachOrderNumber(task?.custom_values));
}

export function validateKolOrderNumber(raw, tasks = [], excludeTaskId = null) {
  const normalized = normalizeKolOrderNumber(raw);
  if (!normalized) {
    const text = String(raw || '').trim();
    return { ok: false, code: text ? 'invalid_format' : 'required' };
  }
  const sequence = parseKolOrderNumberSequence(normalized);
  const conflict = kolOrderNumberUsageBySequence(tasks, { excludeTaskId }).get(sequence);
  if (conflict) {
    return { ok: false, code: 'duplicate', normalized, sequence, conflict };
  }
  const heldByTask = kolOrderNumberHeldByTask(tasks, excludeTaskId);
  if (
    sequence <= KOL_ORDER_NUMBER_EXTERNAL_FLOOR
    && heldByTask !== sequence
  ) {
    return {
      ok: false,
      ...
      normalized,
      sequence,
      floor: KOL_ORDER_NUMBER_EXTERNAL_FLOOR,
      next: suggestNextKolOrderNumber(tasks),
    };
  }
  return { ok: true, normalized, sequence };
}
