import { listActiveTeamMemberNames, listActiveTeamMemberNamesForDepartment } from '@/lib/hub-users';
import { cachedTtl } from '@/lib/server-ttl-cache';
...
const TEAM_MEMBERS_TTL_MS = 60_000;

export async function loadHubTeamMembers() {
  try {
    return await cachedTtl('hub-team-members', 'active-names', TEAM_MEMBERS_TTL_MS, () =>
...
export async function loadMarketingTeamMembers() {
  try {
    return await cachedTtl('hub-team-members', 'marketing-names', TEAM_MEMBERS_TTL_MS, () =>
      listActiveTeamMemberNamesForDepartment('marketing')
    );
  } catch (err) {
    console.error('[loadMarketingTeamMembers]', err);
    return [];
  }
}
...
export async function resolveDepartmentLoaderScope(departmentId, searchParams, fixedTool = '') {
  ...
  const marketingDataTool =
    ...
    && ['kol-pool', 'kol-outreach', 'kol-guidelines', 'kol-tracking', 'preorder-survey'].includes(tool);
