...
import { loadKolPoolForPage, loadKolPoolOutreachForPage } from '@/lib/api/kol-pool-handlers';
...
/** KOL outreach — board-scoped tasks + slim pool rows for cards. */
export async function loadMarketingKolOutreachPage(actor) {
  if (!actor?.ok) {
    return {
      tasks: [],
      tasksFilterKey: null,
  ...
  try {
    try {
      ...
      console.error('[loadMarketingKolOutreachPage] ensure board', boardErr);
    }
    ...
    const [tasks, poolLoaded] = await Promise.all([
      ...
      loadKolPoolOutreachForPage().catch(poolErr => {
        console.error('[loadMarketingKolOutreachPage] kol pool', poolErr);
        return EMPTY_KOL_POOL;
      }),
    ]);
