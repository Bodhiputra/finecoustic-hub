...
async function ensureRelationalSchema() {
  ...
  if (!schemaPromise) {
    schemaPromise = (async () => {
      ...
      await sql()`ALTER TABLE appdev_issues ADD COLUMN IF NOT EXISTS file_urls JSONB NOT NULL DEFAULT '[]'`;
      schemaReady = true;
    })().catch(err => {
      schemaPromise = null;
      throw err;
...
function issueToRow(issue) {
  ...
  return {
    ...
    file_urls: normalized.file_urls || [],
...
export async function migrateBlobToRelationalIfNeeded() {
  ...
  migrationPromise = (async () => {
    ...
    // Relational store is authoritative once it has any rows. Do not re-import from
    // the legacy blob when counts diverge — that resurrects issues deleted in relational.
    if (existing > 0) {
      migrationDone = true;
      return { skipped: true, reason: 'relational_active', issueCount: existing };
    }

    if (issues.length === 0) {
      migrationDone = true;
      return { skipped: true, reason: 'empty_blob', issueCount: 0 };
    }
    ...
    for (const issue of issues) {
      ...
      await sql()`
        INSERT INTO appdev_issues (
          ...
          assigned_at, completed_at, due_at, image_urls, video_urls, file_urls, created_at, updated_at
        ) VALUES (
...
export async function insertRelationalIssue(issue) {
  await ensureReady();
  ...
  await bumpMetaUpdated({ people, task_types });
...
export async function updateRelationalIssue(id, issue) {
  ...
  const rows = await sql()`
    UPDATE appdev_issues SET
      ...
      file_urls = ${JSON.stringify(row.file_urls)}::jsonb,
...
export async function deleteRelationalIssue(id) {
  await ensureReady();
  const rows = await sql()`DELETE FROM appdev_issues WHERE id = ${id} RETURNING id`;
  if (rows.length) await bumpMetaUpdated();
}
...
/** Atomically reserve the next issue number (safe under concurrent creates). */
export async function allocateRelationalNextNumber() {
  await ensureReady();
  const rows = await sql()`
    UPDATE appdev_board_meta
    SET next_number = next_number + 1, updated_at = NOW()
    WHERE id = ${META_KEY}
    RETURNING next_number - 1 AS allocated
  `;
  if (rows.length) return rows[0].allocated;
  await sql()`
    INSERT INTO appdev_board_meta (id, project, next_number, people, task_types, updated_at)
    VALUES (${META_KEY}, 'Finecoustic App Development', 2, '[]', '[]', NOW())
    ON CONFLICT (id) DO NOTHING
  `;
