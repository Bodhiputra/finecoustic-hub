...
import { getTaskAssignees, hasTaskAssignees, isUserAmongTaskAssignees } from '@/lib/task-assignees';

export function isPersonalHubTask(task) {
  return task?.department === PERSONAL_DEPARTMENT_ID;
}
...
export function taskAssigneeKey(task) {
  ...
  return assignees.length ? personKey(assignees[0]) : personKey(task?.assignee);
}
...
export function isTaskAssignee(task, actorName) {
  return isUserAmongTaskAssignees(task, actorName);
}
...
/** New/assigned tasks stay in Todo until moved (claim, drag, or status pick). */
export function applyAutoStartWhenAssigned(task) {
  return task;
}

/** @deprecated Auto-start on assign removed — use claim or manual column move. */
export function shouldApplyAutoStartOnUpdate() {
  return false;
}
...
function canTransitionKolOutreachStatus(task, fromStatus, toStatus, actorName, { isManager = false, isAdmin = false } = {}) {
  ...
  return isTaskAssignee(task, actorName) || isManager || isAdmin || isProtectedAssigner(actorName);
}
...
export function canTransitionTaskStatus(task, fromStatus, toStatus, actorName, { isManager = false, isIntern = false, isAdmin = false } = {}) {
  ...
  if (isKolOutreachTask(task)) {
    return canTransitionKolOutreachStatus(task, from, to, actorName, { isManager, isAdmin });
  }
...
export function resolveWorkflowPatch(existing, patch, actor) {
  ...
  if (patch.workflow_action === 'claim') {
    ...
    nextPatch.assignees = [String(actorName || '').trim()].filter(Boolean);
