...
/** Assigned team tasks skip Todo — assignment means work has started. Personal hub stays in Todo until moved. */
export function applyAutoStartWhenAssigned(task) {
  if (!isWorkflowTask(task)) return task;
  if (isPersonalHubTask(task)) return task;
...
export function shouldApplyAutoStartOnUpdate(existing, next, patch = {}) {
  if (isPersonalHubTask(next)) return false;
