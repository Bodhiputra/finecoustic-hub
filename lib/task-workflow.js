...
export function canTransitionTaskStatus(task, fromStatus, toStatus, actorName, { isManager = false } = {}) {
  ...
  if (to === 'cancelled') return assigner || isManager;
...
export function assertTaskStatusTransition(task, nextStatus, actor, options = {}) {
  ...
  if (!canTransitionTaskStatus(task, from, to, actorName, { isManager })) {
