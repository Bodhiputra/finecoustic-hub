...
import { normalizePersonalJot, sortPersonalJots } from '@/lib/personal-jots-shared';
...
export async function listPersonalJotsForOwner(ownerKey) {
  ...
  if (useDatabase()) {
    ...
    const rows = await sql()`
      ...
      WHERE owner_key = ${key}
      ORDER BY created_at DESC, sort_order DESC, title ASC
    `;
