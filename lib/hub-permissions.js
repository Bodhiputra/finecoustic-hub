...
import { isUserAmongTaskAssignees } from '@/lib/task-assignees';
...
import { showScheduleDashboardForActor } from '@/lib/hub-user-preferences';
...
export function canDeleteTask(actor, task) {
  ...
  if (String(task.board_id || '') === KOL_OUTREACH_BOARD_ID) {
    ...
    if (isUserAmongTaskAssignees(task, actor.displayName)) return true;
    return false;
  }
...
export function hubActorFromClient(profile) {
  ...
  return {
    ...
    showScheduleDashboard:
      hubUser?.permissions?.showScheduleDashboard
      ?? hubUser?.showScheduleDashboard
      ?? true,
  };
}
...
export function hubPermissionsForClient(actor) {
  ...
  return {
    ...
    showScheduleDashboard: showScheduleDashboardForActor(actor),
  };
}
...
export const HUB_PERMISSION_MATRIX = [
  ...
  {
    area: 'Schedule dashboard (hub home)',
    intern: 'Per user (admin toggle)',
    associate: 'Per user (admin toggle)',
    manager: 'Per user (admin toggle)',
  },
  {
