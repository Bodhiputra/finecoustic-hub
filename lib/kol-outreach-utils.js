...
/** Count inbound vs outbound for outreach segmentation (excludes approach-direction filter). */
export function countOutreachApproachDirections(
  tasks,
  { query, initiative, assignee, dealType, platform, country, poolRecords } = {}
) {
  const rows = filterOutreachTasks(tasks, {
    query,
    initiative,
    assignee,
    dealType,
  ...
  for (const task of rows) {
    if (taskApproachDirection(task) === 'inbound') inbound += 1;
    else outbound += 1;
  }
  return { all: rows.length, outbound, inbound };
}
