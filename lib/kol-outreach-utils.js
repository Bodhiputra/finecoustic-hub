...
export function kolCardChips(task, poolRecord, t) {
  ...
  if (country) {
    chips.push({ key: 'country', label: country, className: 'kol-chip-country' });
  }
  if (platform) {
    chips.push({
      key: 'platform',
      label: platform,
      className: `kol-chip ${platformChipClass(platform)}`,
    });
  }
...
export function filterOutreachTasks(tasks, { section, query, initiative, assignee, dealType, needsFollowUpOnly, poolRecords }) {
