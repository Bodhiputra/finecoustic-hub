...
function orderFromTask(task, poolRecord) {
  ...
  const batch = String(cv[KOL_BOARD_PROP.weibinBatchCode] || cv[KOL_BOARD_PROP.orderNumber] || '').trim();
  ...
  return {
    ...
    recipientName: String(poolRecord?.channel_name || task?.title || '').trim(),
...
function fillOrderRow(row, order, productLine, isFirstLine) {
  ...
  setCell(row, COL.product, productLine.product);
}
