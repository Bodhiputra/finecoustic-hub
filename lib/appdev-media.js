...
function isUploadedMediaUrl(url) {
  return (
    ...
    url.startsWith('/api/v1/internal/media/') ||
    /\.(jpe?g|png|mp4|mov|webp)(\?|$)/i.test(url) ||
...
export function isDirectVideoUrl(url) {
  ...
  if (!raw) return false;
  if (raw.startsWith('/api/appdev/media/') || raw.startsWith('/api/v1/internal/media/')) return true;
