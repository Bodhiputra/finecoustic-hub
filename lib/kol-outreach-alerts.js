...
import { listKolOutreachTasksForAlerts, systemUpdateTask } from '@/lib/internal-data';
import {
  KOL_BOARD_PROP,
  kolOutreachBoardUrl,
...
export async function processKolOutreachAlerts(now = new Date()) {
  ...
  return { sent, autoMoved, boardId: KOL_OUTREACH_BOARD_ID };
}
...
export async function migrateKolOutreachLegacyStatuses() {
  ...
  let migrated = 0;
  for (const task of tasks) {
    if (String(task.status || '') !== 'no_reply') continue;
