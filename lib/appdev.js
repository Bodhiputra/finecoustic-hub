...
export const ISSUE_TYPE_MAX_LENGTH = 48;
...
/** Trim and cap free-form or preset issue types. */
export function normalizeIssueType(type) {
  const cleaned = String(type || '')
    .trim()
    .replace(/\s+/g, ' ')
    .slice(0, ISSUE_TYPE_MAX_LENGTH);
  return cleaned || 'task';
}

/** Display label — known slugs use i18n; custom types show as stored. */
export function formatIssueTypeLabel(type, t) {
  const key = normalizeIssueType(type);
  if (ISSUE_TYPES.includes(key)) {
    const label = t(`appdev.type.${key}`);
    if (label) return label;
  }
  return key;
}

/** Map assigner input (label or slug) back to stored type. */
export function issueTypeFromInput(input, t) {
  const raw = normalizeIssueType(input);
  if (ISSUE_TYPES.includes(raw)) return raw;
  for (const slug of ISSUE_TYPES) {
    const label = formatIssueTypeLabel(slug, t);
    if (label && personKey(label) === personKey(raw)) return slug;
  }
  return raw;
}

/** Value shown in the type text field. */
export function issueTypeForInput(type, t) {
  const raw = String(type || '').trim();
  if (!raw) return '';
  if (ISSUE_TYPES.includes(raw)) return formatIssueTypeLabel(raw, t);
  return raw;
}

/** Distinct task types — presets, saved custom, and types used on issues. */
export function mergeTaskTypes(stored = [], issues = [], extras = []) {
  const map = new Map();
  const add = type => {
    if (!String(type ?? '').trim()) return;
    const normalized = normalizeIssueType(type);
    map.set(personKey(normalized), normalized);
  };
  for (const slug of ISSUE_TYPES) add(slug);
  for (const type of stored || []) add(type);
  for (const issue of issues || []) add(issue?.type);
  for (const type of extras || []) add(type);
  return [...map.values()].sort((a, b) => {
    const aPreset = ISSUE_TYPES.includes(a);
    const bPreset = ISSUE_TYPES.includes(b);
    if (aPreset !== bPreset) return aPreset ? -1 : 1;
    return a.localeCompare(b, undefined, { sensitivity: 'base' });
  });
}

/** @deprecated use mergeTaskTypes */
export function collectIssueTypeFilterOptions(issues, stored = []) {
  return mergeTaskTypes(stored, issues);
}
...
export function normalizeIssue(issue) {
  ...
  next.type = normalizeIssueType(next.type);
  ...
  if (next.due_at != null && typeof next.due_at !== 'string') next.due_at = null;
  if (typeof next.created_at !== 'string') {
    next.created_at = typeof next.updated_at === 'string' ? next.updated_at : null;
  }
  ...
  next.file_urls = normalizeFileUrls(next.file_urls);
...
export const APPDEV_ACTION_NOTIFICATION_TYPES = ['review_request'];

/** Action-required notifications stay in the bar until resolved (e.g. review confirmed). */
export function appdevNotificationRequiresAction(item) {
  if (!item?.type || !APPDEV_ACTION_NOTIFICATION_TYPES.includes(item.type)) return false;
