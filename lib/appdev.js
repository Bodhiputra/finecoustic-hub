...
/** Distinct stored types for filters (includes presets even if unused). */
export function collectIssueTypeFilterOptions(issues) {
  const map = new Map();
  for (const slug of ISSUE_TYPES) {
    map.set(personKey(slug), slug);
  }
  for (const issue of issues || []) {
    const type = normalizeIssueType(issue?.type);
    map.set(personKey(type), type);
  }
